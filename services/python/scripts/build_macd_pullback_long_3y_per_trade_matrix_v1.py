"""Build per-trade × per-cell PnL matrix for `macd_pullback_long` (3-year window).

Foundation for the adaptive-exit-policy research. Detects entries once, then
for every (ATR_TF, SL_MULT, TP_MULT) cell on the candidate menu, simulates
the exit per entry and stores the realized pnl_pct in a wide parquet.
...
Cells (30 total):
  15m × {(1.5, 2..6), (2, 2..6)} = 10 cells
  1h  × {(1.5, 3..6, 8), (2, 3..6, 8)} = 10 cells
  4h  × {(1.5, 4..6, 8, 10), (2, 4..6, 8, 10)} = 10 cells

Run from services/python/:
    .venv/bin/python scripts/build_macd_pullback_long_3y_per_trade_matrix_v1.py
"""
...
# Each tuple is (atr_tf, sl_mult, tp_mult)
...
MODEL_STEM = "macd_pullback_long_histgbm_3y_v1"
FEATURES_PARQUET = (
...
def build_run_id() -> str:
    ...
    return f"adaptive_exit_per_trade_matrix_v1_{short_sha}_{ts}"
...
def main() -> None:
    ...
    asset_atrs: dict[str, dict] = {}  # cached per-asset ATR series
    ...
    for k_idx, asset in enumerate(asset_list):
        ...
        if len(d_close) >= 30:
            dm = _macd(d_close)
            db_arr = ((dm["histogram"] > 0) & (dm["macd"] > dm["signal"]))\
                .reindex(ts_idx, method="ffill").fillna(False).to_numpy(dtype=bool)
        else:
            db_arr = np.zeros(n, dtype=bool)

        # 4h MACD cross-up ≥2 prior below
        h4_close = s_close.resample("4h").last().dropna()
        if len(h4_close) >= 30:
            hm = _macd(h4_close)
            h4_above = (hm["macd"] > hm["signal"]).astype(bool)
            cross = h4_above & ~h4_above.shift(1).fillna(False) & ~h4_above.shift(2).fillna(False)
            cu_full = cross.reindex(ts_idx, method="ffill").fillna(False)
        ...
        raw = [i for i in range(n) if h4_cross_up_15m[i] and db_arr[i]]
        ...
        for ei in ent_idxs:
            ...
            entry_time = pd.Timestamp(ts[ei]).tz_localize("UTC") if pd.Timestamp(ts[ei]).tzinfo is None else pd.Timestamp(ts[ei])
            try:
    ...
    # Sanity check vs prior 3y baseline (15m, SL=2, TP=6 ≈ live cell, not in baseline grid;
    # use closest cell (15m, SL=2, TP=5) which IS in this grid AND was in prior sweep).
    sanity_col = cell_col("15m", 2.0, 5.0)
    sanity_sum = float(out[sanity_col].sum() * 100)
    sanity_n = int(out[sanity_col].notna().sum())
    print(f"\nSANITY: cell {sanity_col}  n={sanity_n}  total_pnl%={sanity_sum:.1f}")
    print(f"        prior R:R sweep had this cell at total_pnl%={4173.0:.1f} (flat) — must match closely")
