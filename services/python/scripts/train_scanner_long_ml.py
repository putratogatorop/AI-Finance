"""Train LightGBM classifier for Scanner Long signals.

Walk-forward validation on blind backtest trades.
Loads from scanner_long_blind table, trains model, sweeps thresholds,
saves best model and re-runs backtest with ML filter.

Run from services/python/:
    python scripts/train_scanner_long_ml.py
"""
...
MODEL_PATH = Path("models/scanner_long_ml.joblib")
...
ML_THRESHOLDS = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]
...
def load_trades() -> pd.DataFrame:
    """Load all trades from scanner_long_blind table."""
    query = "SELECT * FROM scanner_long_blind ORDER BY signal_time"
    df = pd.read_sql(query, engine)
    df["signal_time"] = pd.to_datetime(df["signal_time"])
    df = df.sort_values("signal_time").reset_index(drop=True)
    print(f"Loaded {len(df)} trades from scanner_long_blind")
...
def walk_forward(df: pd.DataFrame):
    ...
    all_oos = []  # Collect OOS predictions from each fold
    fold_results = []
...
def find_best_threshold(all_oos: list[pd.DataFrame]) -> float:
    ...
    # Use only the non-overlapping OOS: Fold 1 test (60-80%) + Fold 2 test (80-100%)
    # But Fold 2 test (80-100%) is the cleanest since Fold 1 covers 60-80%.
    # Combine all OOS and deduplicate by signal_time + symbol.
...
def save_filtered_trades(combined_oos: pd.DataFrame, threshold: float):
    """Save OOS trades passing ML filter to scanner_long_ml_filtered table."""
    ...
    with engine.connect() as conn:
        conn.execute(text("DROP TABLE IF EXISTS scanner_long_ml_filtered"))
        conn.commit()
    save_df.to_sql("scanner_long_ml_filtered", engine, if_exists="replace", index=False)
    print(f"\nSaved {len(save_df)} filtered OOS trades to scanner_long_ml_filtered "
...
def print_comparison(df: pd.DataFrame, combined_oos: pd.DataFrame, threshold: float):
    ...
    print(f"\n{'=' * 70}")
    print("COMPARISON: Raw vs ML-Filtered")
    print(f"{'=' * 70}")
...
def main():
    start_t = time.time()
    print("Scanner Long ML Training")
    print("=" * 70)
