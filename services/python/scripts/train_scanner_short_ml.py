"""Train LightGBM classifier for Scanner Short signals.

Walk-forward validation on blind backtest trades.
Loads from scanner_short_blind table, trains model, sweeps thresholds,
saves best model and re-runs backtest with ML filter.

Run from services/python/:
    python scripts/train_scanner_short_ml.py
"""
...
MODEL_PATH = Path("models/scanner_short_ml.joblib")
...
def load_trades() -> pd.DataFrame:
    """Load all trades from scanner_short_blind table."""
    query = "SELECT * FROM scanner_short_blind ORDER BY signal_time"
    df = pd.read_sql(query, engine)
    df["signal_time"] = pd.to_datetime(df["signal_time"])
    df = df.sort_values("signal_time").reset_index(drop=True)
    print(f"Loaded {len(df)} trades from scanner_short_blind")
...
def save_filtered_trades(combined_oos: pd.DataFrame, threshold: float):
    """Save OOS trades passing ML filter to scanner_short_ml_filtered table."""
    ...
    with engine.connect() as conn:
        conn.execute(text("DROP TABLE IF EXISTS scanner_short_ml_filtered"))
        conn.commit()
    save_df.to_sql("scanner_short_ml_filtered", engine, if_exists="replace", index=False)
    print(f"\nSaved {len(save_df)} filtered OOS trades to scanner_short_ml_filtered "
...
def main():
    start_t = time.time()
    print("Scanner Short ML Training")
    print("=" * 70)
