"""Live scanner — macd_early_trend_short detector (Week 8 v8 BALANCED, locked).

Symmetric mirror of macd_early_trend_long. Fires a SHORT entry when:
  1. Daily MACD(12,26,9) is FRESHLY bear — i.e., the bear-flip (hist was ≥ 0,
     now hist < 0 AND macd < signal) occurred within the last 10 days.
  2. 4h MACD(12,26,9) cross-DOWN after ≥ 2 prior 4h bars above signal.
  3. Entry on the first 15m bar of the new 4h period where both conditions met.
  4. 24h per-symbol cooldown.

"Fresh" bear is what distinguishes this from macd_pullback_short: it captures
the early phase of a downtrend where momentum is still accelerating, before the
bear signal becomes stale and mean-reversion tends to occur.

Validated in portfolio 92373355_20260429T041931Z:
  per-detector wf_p5 = 2.054, holdout PF inside BALANCED 4-det portfolio.
  Portfolio-level wf_p5 = 1.486 alongside macd_pullback_short_e2,
  macd_pullback_long_e2, rsi_recovery_long_e2.

Usage from services/python/:
    python scripts/live_scanner_macd_early_trend_short.py
"""
...
EARLY_TREND_FRESH_BARS = 10 * 96  # 10 days of 15m bars (locked spec)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/live_scanner_macd_early_trend_short.log", mode="a"),
    ],
)
logger = logging.getLogger("scanner_macd_et_short")
...
def detect_macd_early_trend_short(df: pd.DataFrame, btc_score: float) -> dict | None:
    """Evaluate macd_early_trend_short at the latest 15m bar.

    Same gates as macd_pullback_short PLUS freshness filter:
    the daily bear-flip (was-not-bear → is-bear) must be within the last 10 days.
    """
    if len(df) < 4 * 96:
        return None
    ...
    # Freshness — find most recent bear-flip (False→True) in daily_bear_d
    bear_arr = daily_bear_d.to_numpy(dtype=bool)
    flip_idx = []
    for i in range(1, len(bear_arr)):
        if bear_arr[i] and not bear_arr[i - 1]:
            flip_idx.append(i)
    if not flip_idx:
        # No bear-flip ever — not fresh
        return None
    last_flip_daily_pos = flip_idx[-1]  # position in daily index

    # Is current bar within EARLY_TREND_FRESH_BARS (10d × 96 15m bars) of the flip?
    # Convert: flip_date = d_close.index[last_flip_daily_pos]
    flip_date = d_close.index[last_flip_daily_pos]
    # Convert to timestamp for comparison
    if flip_date.tzinfo is None:
        flip_ts = pd.Timestamp(flip_date, tz="UTC")
    else:
        flip_ts = pd.Timestamp(flip_date)
    last_15m_ts = ts_idx[-1]
    bars_since_flip = (last_15m_ts - flip_ts).total_seconds() / (15 * 60)
    if bars_since_flip > EARLY_TREND_FRESH_BARS:
        return None

    # 4h MACD cross-down (same as pullback_short)
    ...
    if h4_idx < 2:
        return None
    if not bool(h4_cross_down.iloc[h4_idx]):
        return None

    # Daily bear check at current day
    ...
    return {
        ...
        "btc_score": btc_score,
        "bars_since_bear_flip": float(bars_since_flip),
    }
...
def ensure_table(engine):
    with engine.begin() as conn:
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS scanner_signals_macd_early_trend_short (
                ...
                bars_since_bear_flip DOUBLE PRECISION,
                created_at TIMESTAMPTZ DEFAULT NOW(),
                UNIQUE (symbol, direction, signal_time)
            )
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_macdet_signal_time
            ON scanner_signals_macd_early_trend_short (signal_time DESC)
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_macdet_active
            ON scanner_signals_macd_early_trend_short (status, signal_time)
            WHERE status = 'active'
        """))
...
def write_signal(engine, *, hit: dict) -> bool:
    try:
        with engine.begin() as conn:
            conn.execute(text("""
                INSERT INTO scanner_signals_macd_early_trend_short
                (symbol, direction, signal_time, status,
                 entry_price, atr14_at_entry,
                 daily_macd, daily_signal, daily_hist,
                 h4_macd, h4_signal, h4_hist, btc_score, bars_since_bear_flip)
                VALUES (:symbol, :direction, :sig_t, 'active',
                        :ep, :atr, :dm, :ds, :dh, :hm, :hs, :hh, :bs, :bsf)
...
def main() -> None:
    logger.info("=" * 60)
    logger.info("LIVE SCANNER — macd_early_trend_short (Week 8 v8 BALANCED, locked)")
    logger.info("=" * 60)
    logger.info(
        f"Detector: FRESH daily MACD bear (≤{EARLY_TREND_FRESH_BARS // 96}d since bear-flip) "
        f"AND 4h MACD cross-down after ≥{H4_BARS_PRIOR_ABOVE} prior bars above signal, "
        f"cooldown {COOLDOWN_SECONDS // 3600}h"
    )

    engine = get_db_engine()
    ensure_table(engine)
    logger.info("scanner_signals_macd_early_trend_short table ready")
    ...
    while True:
        try:
            ...
            for pair, ticker in tickers.items():
                ...
                hit = detect_macd_early_trend_short(df, btc_score_cached)
                if hit is None:
                    continue
                hit["symbol"] = symbol
                ...
                if ok:
                    ...
                    logger.info(
                        ...
                        f"bars_since_flip={hit['bars_since_bear_flip']:.0f}"
                    )
