...
# --- PARAMS — NEW macro-regime gate (BTC 4h close < BTC 4h EMA-50) ---
BTC_MACRO_TIMEFRAME = "4h"    # resample 15m closes to 4h
BTC_MACRO_EMA_SPAN = 50       # 50-period EMA on the resampled series
...
def _build_btc_macro_downtrend(candles: pd.DataFrame) -> pd.Series:
    """At each 15m timestamp, True when BTC's most recent CLOSED 4h bar is
    below its 50-period 4h EMA computed over only past closed 4h bars.

    Strict no-lookahead: the resampled 4h close at boundary `t` becomes
    available only AT `t` (the bar closes at `t`); we shift by one 4h bar
    so the indicator at any 15m timestamp uses only data fully available
    by then. The 15m index is then forward-filled.
    """
    btc = (
        candles[candles["asset"] == BTC_ASSET]
        .sort_values("timestamp")
    ...
    # Resample to 4h (label=right means the bar at HH=04 contains data 00:00-04:00).
    btc_4h = btc.resample(BTC_MACRO_TIMEFRAME, label="right", closed="right").last()
    ema = btc_4h.ewm(span=BTC_MACRO_EMA_SPAN, adjust=False).mean()
    downtrend_4h = (btc_4h < ema).shift(1)  # shift to ensure no lookahead at the boundary
    # Forward-fill the 4h boolean to the 15m grid
    return downtrend_4h.reindex(btc.index, method="ffill").fillna(False)
...
def _run_strategy_b_filtered(candles: pd.DataFrame) -> pd.DataFrame:
    """Detect Strategy B entries, gate on btc_flat + low vol decile + BTC 4h downtrend."""
    print("[1/4] computing BTC 96-bar regime series...", flush=True)
    btc_regime = _build_btc_regime_series(candles)

    print("[2/4] computing BTC 4h EMA-50 macro downtrend gate...", flush=True)
    btc_macro_down = _build_btc_macro_downtrend(candles)

    print("[3/4] computing cross-sectional vol-decile panel...", flush=True)
    decile_panel = _build_vol_decile_panel(candles)

    print("[4/4] running Strategy B per asset, filtering, simulating shorts...",
          flush=True)
    rows = []
    ...
    for idx, asset in enumerate(assets):
        ...
        for eb in _detect_b_entries(close, high, volume):
            ...
            # Filter 1: BTC 96-bar regime must be flat
            try:
            ...
            # Filter 2: BTC 4h close < 4h EMA-50 (macro downtrend)
            try:
                if not bool(btc_macro_down.loc[entry_ts]):
                    continue
            except KeyError:
                continue

            # Filter 3: asset's cross-sectional vol decile <= VOL_DECILE_MAX
            try:
