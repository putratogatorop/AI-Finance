"""Build per-trade × per-cell PnL matrix for `macd_pullback_short` (3-year window).

Clone of build_macd_pullback_long_3y_per_trade_matrix_v1.py adapted for the
short side: daily MACD bear + 4h MACD cross-DOWN ≥2 prior above.
...
Run from services/python/:
    .venv/bin/python scripts/build_macd_pullback_short_3y_per_trade_matrix_v1.py
"""
...
MODEL_STEM = "macd_pullback_short_histgbm_3y_v1"
FEATURES_PARQUET = (
...
def build_run_id() -> str:
    ...
    return f"adaptive_exit_per_trade_matrix_short_v1_{short_sha}_{ts}"
...
def main() -> None:
    ...
    for k_idx, asset in enumerate(asset_list):
        ...
        raw = [i for i in range(n) if h4_cross_down_15m[i] and dbear_arr[i]]
