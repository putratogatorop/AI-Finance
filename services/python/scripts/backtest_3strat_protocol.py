...
# --- PARAMS (ported from backtest_scanner_short_3strat.py) ---
# Entry gates
VOL_MA_PERIOD = 20
VOL_SPIKE_A = 3.0
VOL_SPIKE_B = 2.0
...
MIN_BARS_PER_ASSET = VOL_MA_PERIOD + max(PRICE_LOOKBACK_A, PRICE_LOOKBACK_B, PRICE_LOOKBACK_C) + 10
...
def _simulate_short_trade(
    ...
    """Short-side trade simulation. SL hits first if triggered in same bar.

    Ported verbatim (logic + friction) from backtest_scanner_short_3strat.py:simulate_trade.
    """
    n = len(close)
    if entry_bar + 1 >= n:
        return None
...
def _detect_a(close, high, volume) -> list[int]:
    """Strategy A (vol_accel): vol ≥ 3× MA20, price already ≥5% off 96-bar high."""
    n = len(close)
    vol_ma = pd.Series(volume).rolling(VOL_MA_PERIOD).mean().to_numpy()
    rolling_high = pd.Series(high).rolling(PRICE_LOOKBACK_A).max().shift(1).to_numpy()
    entries: list[int] = []
    cooldown_until = 0
    begin = VOL_MA_PERIOD + PRICE_LOOKBACK_A
...
def _detect_b(close, high, volume) -> list[int]:
    """Strategy B (wait_bounce): price down ≥5% in 32-bar window + any ≥2× vol spike in window."""
    n = len(close)
    vol_ma = pd.Series(volume).rolling(VOL_MA_PERIOD).mean().to_numpy()
    ...
    # Window for "any vol spike" is j in [i - PRICE_LOOKBACK_B, i]: 33 bars inclusive.
...
def _detect_c(close, high, open_, volume) -> list[int]:
    """Strategy C (multi_frame): approximates volume_breakouts direction=-1 signal.

    Original C required a row in the volume_breakouts table. That table isn't
    present in the snapshot, so we approximate the same rule the scanner uses
    to populate it: vol_ratio ≥ 2 AND bar is red (close<open) — i.e. a down-
    directional volume event — AND price already ≥5% off 96-bar high.
    """
    n = len(close)
    vol_ma = pd.Series(volume).rolling(VOL_MA_PERIOD).mean().to_numpy()
    rolling_high = pd.Series(high).rolling(PRICE_LOOKBACK_C).max().shift(1).to_numpy()
    entries: list[int] = []
    cooldown_until = 0
    begin = VOL_MA_PERIOD + PRICE_LOOKBACK_C
    for i in range(begin, n):
        if i < cooldown_until:
            continue
        vm = vol_ma[i]
...
def _run_all_strategies(
    ...
    """Sweep every asset in the snapshot, emit trades per strategy."""
    trades_by_strat: dict[str, list[dict]] = {"A": [], "B": [], "C": []}
    ...
    for idx, asset in enumerate(assets):
        ...
        for eb in _detect_a(close, high, volume):
            ...
            trades_by_strat["A"].append(t)

        for eb in _detect_b(close, high, volume):
            t = _simulate_short_trade(close, high, low, eb)
            if t is None:
                continue
            t["direction"] = "short"
            ...
            t["strategy"] = "B"
            t["entry_time"] = str(timestamps[eb])
            trades_by_strat["B"].append(t)

        for eb in _detect_c(close, high, open_, volume):
            t = _simulate_short_trade(close, high, low, eb)
            if t is None:
                continue
            t["direction"] = "short"
            ...
            t["strategy"] = "C"
            t["entry_time"] = str(timestamps[eb])
            trades_by_strat["C"].append(t)

        if (idx + 1) % 25 == 0:
            print(
                f"  [{idx + 1}/{len(assets)}] "
                f"A={len(trades_by_strat['A']):,}  "
                f"B={len(trades_by_strat['B']):,}  "
                f"C={len(trades_by_strat['C']):,}",
                flush=True,
            )

    empty_cols = ["direction", "pnl_pct", "bars_held"]
    return {
        k: (pd.DataFrame(v) if v else pd.DataFrame(columns=empty_cols))
        for k, v in trades_by_strat.items()
    }
...
def main() -> None:
    ...
    trades_by_strat = _run_all_strategies(candles)

    # Snapshot shas are read from the manifest once, reused across strategies.
    ...
    # One run_id per strategy so each has its own citable results/<run_id>/.
    ...
    for strat in ("A", "B", "C"):
        run_id = f"{script_stem}_{strat}_{sha8}_{ts}"
        trades_df = trades_by_strat[strat]
        ...
        print(f"[{strat}] run_id={run_id}  metrics={metrics}")
