"""Phase 15 — ATR-based detector vs the bigmover-volume baseline.

User's hypothesis: the bigmover-volume detector triggers on raw % moves
+ raw volume spikes, which mean different things on BTC (low vol, 5%
drop is rare and significant) vs alts (high vol, 5% drop is daily noise).
An ATR-normalized trigger should adapt to each asset's volatility regime.

Three detectors compared head-to-head, same SHORT-side exits + sizing:

  D0 — bigmover-vol (current):
       vol_ratio >= 3.0
     & (rolling_high_96 - close) / rolling_high_96 >= 0.05
     & price_drop > price_rise

  D1 — atr-normalized drop + vol confirm:
       (rolling_high_96 - close) / ATR_14 >= 2.5
     & vol_ratio >= 2.0
     & close < open  (red current bar)

  D2 — ATR-scaled drop with vol ratio (asset-relative):
       (rolling_high_96 - close) / close >= 3.0 * (ATR_14 / close)
     & vol_ratio >= 2.0

Each detector emits short trades with cooldown=96 bars; same simulation
(SL=5% / TP=15% / timeout=672 / friction=0.0015) and the same v2-style
sizing applied (BTC trend score sign-locked, 1.5x cap).

This test does NOT use the v2 ML classifier (it was trained on the D0
distribution; new detectors need their own classifier). We compare
RAW detector quality first; if D1 or D2 looks materially better than
D0 on raw metrics, the next step is to retrain a classifier on it.

Run from services/python/:
    python scripts/phase15_atr_detector_vs_volume.py
"""
...
OUT_PATH = REPO_ROOT / "services/python/results" / "phase15_atr_detector_vs_volume.json"
...
def _detect_short_d0_volume(close, high, low, open_, volume) -> np.ndarray:
    """D0 — bigmover-volume baseline."""
    n = len(close)
    ...
    for i in range(PRICE_LOOKBACK + VOL_MA_PERIOD, n):
        ...
        if rolling_high[i] <= 0:
            continue
        price_drop = (rolling_high[i] - close[i]) / rolling_high[i]
        if price_drop < 0.05:
            continue
        price_rise = (close[i] - rolling_low[i]) / rolling_low[i] if rolling_low[i] > 0 else 0
        if not (price_drop > price_rise):
            continue
        out[i] = True
    return out
...
def _detect_short_d1_atr_drop(close, high, low, open_, volume) -> np.ndarray:
    """D1 — ATR-normalized drop + lower vol confirm + red current bar."""
    n = len(close)
    out = np.zeros(n, dtype=bool)
...
def _detect_short_d2_atr_scaled(close, high, low, open_, volume) -> np.ndarray:
    """D2 — drop scaled to asset's typical ATR% on current price."""
    n = len(close)
    out = np.zeros(n, dtype=bool)
    if n < PRICE_LOOKBACK + ATR_PERIOD:
        return out
    ...
    for i in range(PRICE_LOOKBACK + VOL_MA_PERIOD, n):
        if vol_ma[i] <= 0 or volume[i] / vol_ma[i] < 2.0:
            continue
        if not (close[i] > 0 and atr14[i] > 0):
            continue
        atr_pct = atr14[i] / close[i]
        if rolling_high[i] <= 0:
            continue
        price_drop = (rolling_high[i] - close[i]) / rolling_high[i]
        if price_drop < 3.0 * atr_pct:
            continue
        price_rise = (close[i] - rolling_low[i]) / rolling_low[i] if rolling_low[i] > 0 else 0
        if not (price_drop > price_rise):
            continue
...
def _run_detector(
    ...
    *,
    ...
    """Run detector across all assets, sized by BTC trend score (sign-locked SHORT),
    with rapid-rally exit applied. Returns trade DataFrame."""
    rows = []
...
def main():
    ...
    detectors = {
        "D0_bigmover_volume":   _detect_short_d0_volume,
        "D1_atr_drop_2.5_vol_2": _detect_short_d1_atr_drop,
        "D2_atr_scaled_drop":    _detect_short_d2_atr_scaled,
    }

    sweep = {}
    print("\n=== Running 3 detectors (same exits + sizing + rapid_rally) ===")
    for name, fn in detectors.items():
        print(f"\n  detector: {name}", flush=True)
        sim = _run_detector(candles, fn, btc_score_15m=btc_score_15m, rapid_exit_15m=rapid_exit_15m)
        if len(sim) == 0:
