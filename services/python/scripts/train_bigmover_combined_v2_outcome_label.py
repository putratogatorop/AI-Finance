...
MODEL_OUT = MODELS_DIR / "bigmover_combined_v2_outcome.joblib"
META_OUT = MODELS_DIR / "bigmover_combined_v2_outcome_meta.json"
SIZING_OUT = MODELS_DIR / "bigmover_combined_v2_outcome_sizing.json"

# v2 label: actual trade outcome under SL=5%/TP=15%/timeout=672 — i.e.,
# label = 1 iff pnl_pct > 0 (the trade was a winner net of friction).
# This aligns the classifier's training signal with what the strategy
# actually rewards, instead of the v1 proxy (10%-drop-in-48h-before-4%-rally).
LABEL_MAX_BARS = 192  # kept only for filter_labelable's horizon check; not used by the v2 label
USE_OUTCOME_LABEL = True
...
def main() -> None:
    ...
    # ---- v2 outcome-aligned label: trade was a winner net of friction ---
    # No horizon-fit filter needed — every Phase-2 trade already has its
    # final pnl_pct from the backtest's SL/TP/timeout exit logic. The label
    # is the actual outcome, not a proxy.
    trades["label"] = (trades["pnl_pct"] > 0).astype(int)
    pos_rate = float((trades["label"] == 1).mean())
    print(f"  v2 outcome-label positive rate: {pos_rate:.1%}", flush=True)
    ...
    print(f"    short positive rate: {pos_short:.1%}    long positive rate: {pos_long:.1%}",
          flush=True)
    # For comparison vs v1 label: a trade can win the v1 label
    # (10%-drop-in-48h-before-4%-rally) but lose the v2 label (and vice
    # versa). Compute the agreement rate so we know how different the
    # signals are.
    keep_v1 = filter_labelable(trades, candles["timestamp"].max(), max_bars=LABEL_MAX_BARS)
    if keep_v1.any():
        v1_labels = make_label(
            trades[keep_v1], candles,
            fwd_target=0.10, drawdown_cap=0.04, max_bars=LABEL_MAX_BARS,
        )
        valid = v1_labels != -1
        if valid.any():
            v1 = v1_labels[valid].astype(int).to_numpy()
            v2 = trades[keep_v1].loc[valid.index[valid]]["label"].astype(int).to_numpy()
            agree = float((v1 == v2).mean())
            print(f"  v1-vs-v2 label agreement rate (subset where v1 is computable): {agree:.1%}",
                  flush=True)
    ...
    meta = {
        ...
        "label_kind": "outcome",                # v2: label = pnl_pct > 0
        "label_thresholds": {
            "kind": "outcome_pnl_positive",
            "exit_sl": 0.05,
            "exit_tp": 0.15,
            "exit_timeout": 672,
            "exit_friction": 0.0015,
        },
